...
namespace MyXafSolution.Module.E2E.Tests;

public class MyXafSolutionTests : IDisposable {
    const string BlazorAppName = "MyXafSolutionBlazor";
    const string WinAppName = "MyXafSolutionWin";
    const string AppDBName = "MyXafSolution";
    EasyTestFixtureContext FixtureContext { get; } = new EasyTestFixtureContext();

	public MyXafSolutionTests() {
        FixtureContext.RegisterApplications(
            new BlazorApplicationOptions(BlazorAppName, string.Format(@"{0}\..\..\..\..\MyXafSolution.Blazor.Server", Environment.CurrentDirectory)),
            new WinApplicationOptions(WinAppName, string.Format(@"{0}\..\..\..\..\MyXafSolution.Win\bin\EasyTest\net6.0-windows\MyXafSolution.Win.exe", Environment.CurrentDirectory))
        );
        FixtureContext.RegisterDatabases(new DatabaseOptions(AppDBName, "MyXafSolutionEasyTest", server: @"(localdb)\mssqllocaldb"));	           
	}
    public void Dispose() {
